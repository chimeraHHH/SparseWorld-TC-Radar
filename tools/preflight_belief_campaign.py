...
os.chdir(code)
root = Path('/storage/data/metaiot_data/huayiming/SparseWorld/analysis/belief_campaign_20260920')
...
try:
    with (root/'cpu_tests.log').open('wb') as log:
        subprocess.run([sys.executable, '-m', 'pytest', '-q', 'tests/test_radar_belief.py', 'tests/test_forecast_improvements.py',
    ...
    for arm in ('belief', 'camera'):
        with (root/('cpu_'+arm+'.log')).open('wb') as log:
            subprocess.run([sys.executable, 'tools/check_belief_contracts.py', '--device', 'cpu',
